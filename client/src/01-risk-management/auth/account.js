...
export default function () {
  ...
  const renderAccount = accounts => {
    const as = Array.isArray(accounts) ? accounts : [accounts]
    as.forEach(convertTimestamp)
    return <MaterialTable title="Accounts" columns={columns} data={as} />
  }
  ...
  return (
    <>
      <Fetcher action="/auth/account">{data => renderAccount(data)}</Fetcher>
